...
public class PIDBase extends SendableBase implements PIDInterface, PIDOutput {
  ...
  private double m_IRange = 0;

  private Field[] k_fields = {
    new Field("result", 'f'),
    new Field("P", 'f'),
    new Field("Total Error", 'f'),
    new Field("D", 'f'),
    new Field("Error", 'f'),
    new Field("Feed Forward", 'f'),
  };

  // private Field[] k_fields = {
  //   new Field("P * error", 'f'),
  //   new Field("I * totalError", 'f'),
  //   new Field("D * (error-prevError)", 'f'),
  //   new Field("result", 'f'),
  //   new Field("feed forward", 'f'),
  //   new Field("setpoint", 'f'),
  //   new Field("input", 'f'),
  //   new Field("P", 'f'),
  //   new Field("I", 'f'),
  //   new Field("D", 'f'),
  //   new Field("F", 'f'),
  // };
  private CSVWriter writer = null; // = new CSVWriter(getClass().getName(), k_fields);
  ...
  static int m_writerCount = 0;

  protected void writerStart(){
    writer = new CSVWriter(m_writerCount++ + "-" + getClass().getName(), k_fields);
    // writer.start();
  }

  protected void writerFinish(){
    if (writer != null)
    {
      // writer.finish();
      writer = null;
  ...
  protected void calculate() {
    ...
    if (enabled) {
      ...
      if (pidSourceType.equals(PIDSourceType.kRate)) {
        ...
        m_thisMutex.lock();
        try {
          // writer.write(result, P, totalError, D, error, feedForward);     
        } finally {
          m_thisMutex.unlock();
        }
        ...
        if (iRange != 0)
        {
          // System.out.println(String.format("iRange=%f,error=%f", iRange, error));
      ...
      // writer.write(P * error, I * totalError, D * (error - prevError), result, feedForward, m_setpoint, input, P, I, D, m_F);
      ...
      try {
        ...
        try {
          if (m_enabled) {
            ...
            // System.out.println(String.format("error=%f, power=%f", error, result));
  ...
  protected double calculateFeedForward() {
    if (m_pidInput.getPIDSourceType().equals(PIDSourceType.kRate)) {
      ...
      return -m_F * Math.cos(Math.toRadians(ArmSubsystem.ticksToDeg(getSetpoint())));
    }
  }
