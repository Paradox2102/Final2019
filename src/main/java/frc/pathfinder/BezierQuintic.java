...
//		double angle = Math.atan2(dy, dx);
		...
		m_p2.m_x = m_p1.m_x = m_p0.m_x + d * Math.cos(angle1);
		m_p2.m_y = m_p1.m_y = m_p0.m_y + d * Math.sin(angle1);
...
//		double angle = Math.atan2(dy, dx) * scale;
		...
		m_p4.m_x = m_p3.m_x = m_p5.m_x + d * Math.cos(angle2);
		m_p4.m_y = m_p3.m_y = m_p5.m_y + d * Math.sin(angle2);
		
	}
	
	BezierQuintic(double p0x, double p0y, double angle1, double l1, double p3x, double p3y, double angle2, double l2)
	{
		double dx = l1 * Math.cos(angle1);
		double dy = l1 * Math.sin(angle1);
